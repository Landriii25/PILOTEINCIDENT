...
@section('content')
    {{-- Flash & erreurs (si tu utilises mes composants, sinon garde les 2 blocs ci-dessous) --}}
    @if(session('success'))
        ...
        <div class="row">
            @foreach($applications as $app)
                @php
                    ...
                    // URL logo/miniature (supporte "storage/..." ou URL absolue)
                    $thumb = $app->thumb_url ?: $app->logo_url;
                ...
                <div class="col-sm-6 col-md-4 col-lg-3 mb-4">
                    <div class="app-card h-100 position-relative">
                        <div class="app-card-media">
                            @if($thumb)
                                <img src="{{ asset($thumb) }}" alt="{{ $app->nom }}">
                            @else
...
<style>
    ...
    .app-card:hover{
        transform:translateY(-6px); box-shadow:0 16px 40px rgba(31,78,255,.18);
    }
    .app-card-media{
    ...
    .app-overlay{
        position:absolute; inset:0; background:linear-gradient(180deg, rgba(2,0,36,0) 20%, rgba(0,0,0,.35) 100%);
