        ...
        <form method="POST" action="{{ route('applications.store') }}" enctype="multipart/form-data">
            ...
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <select name="statut" class="form-control" required>
                        @foreach(['Actif','En maintenance','Retirée'] as $st)
                            <option value="{{ $st }}" @selected(old('statut')==$st)>{{ $st }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <small class="form-text text-muted">Une miniature 256×256 sera générée automatiquement.</small>
                    <div class="mt-2" id="preview" style="display:none;">
                        <img class="rounded shadow-sm" style="width:96px;height:96px;object-fit:cover" />
                    </div>
                </div>
            </div>
